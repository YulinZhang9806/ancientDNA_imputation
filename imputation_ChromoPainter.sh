...
$vcftools --gzvcf chr$chrom.1kg.phase3.v5a.vcf.gz --remove-indels --min-alleles 2 --max-alleles 2 --maf 0.01 --recode --recode-INFO-all --out chr$chrom.filtered
...
-R $hg19ref \
-I ${root}_1x.bam \
-L chr$chromo.filtered.recode.vcf \
--output_mode EMIT_ALL_SITES \
--allSitePLs \
--alleles chr$chromo.filtered.recode.vcf \
...
$vcftools --vcf ${root}_1x.1240k.vcf --chr $chrom --recode --recode-INFO-all --out ${root}_1x.1240k.chr$chrom
###split input vcf for quicker imputation
cat ${root}_1x.1240k.chr$chrom.recode.vcf | java -jar splitvcf.jar $chrom 5000 3000 ${root}_1x.1240k.chr$chrom.beagle
###beagle
...
java -Xmx120g -jar $beagle gprobs=true impute=true gl=${root}_1x.1240k.chr$chrom.beagle.$input.vcf.gz ref=chr$chrom.filtered.recode.vcf map=plink.chr$chrom.GRCh37.map excludesamples=non.eur.excl excludemarkers=non1240k.excl chrom=chr$chrom:$start-$end out=${root}_1x.1240k.chr$chrom.impute.$input
...
$vcftools --vcf $root.$chrom.vcf --min-meanDP 15 --recode --recode-INFO-all --out $root.$chrom.filtered
###split vcf files with GP thresholds
python threshold_imputation_vcf.py
###compare high coverage file with imputed files (if to extract 1240k, change the reference file compared to with $root.1240k.$chrom.filtered.recode.vcf)
gzip -d ${root}_1x.1240k.chr$chrom.impute.vcf.gz
java -jar /home/yulin_zhang/SnpSift.jar concordance -v $root.$chrom.filtered.recode.vcf ${root}_1x.1240k.chr$chrom.impute.vcf
###making figures for accuracy evaluation
python plot_imputation_whole.py

---------------------------GLIMPSE_imputation--------------------------
#chunking reference chromosome into pieces
bgzip chr$chrom.filtered.recode.vcf
$bcftools index chr$chrom.filtered.recode.vcf.gz
/home/yulin_zhang/GLIMPSE_chunk --input chr$chrom.filtered.recode.vcf.gz --region $chrom --window-size 10000000 --buffer-size 1000000 --output chunks.$chrom.txt
#imputation
...
> /home/yulin_zhang/GLIMPSE_phase --input ${root}_1x.1240k.chr$chrom.vcf.gz --reference chr$chrom.filtered.vcf.gz --map /GLIMPSE/maps/genetic_maps.b37/$chrom.b37.gmap.gz --input-region $IRG --output-region $ORG --output ${root}_1x.1240k.impute.$input.chr$chrom.vcf --thread 20
...
###run fineStructure
./fs Worldpop_capture_$chrom.cp -phasefiles Worldpop_capture.filsnp.$chrom.phase -recombfiles Worldpop_capture.filsnp.$chrom.recombfile -idfile Worldpop_capture.id -s1minsnps 5000 -s3iters 10000 -s4iters 10000 -numthreads 10 -go
